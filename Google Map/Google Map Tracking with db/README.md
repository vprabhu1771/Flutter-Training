...
class _GoogleMapTrackingScreenState extends State<GoogleMapTrackingScreen> {
  ...
  static const int SAVE_INTERVAL_SECONDS = 60; // 1 minute
  ...
  Future<void> _saveLocationToDatabase(double latitude, double longitude) async {
    ...
    debugPrint("Saved Location at ${DateTime.now()}: $latitude, $longitude");
  ...
  void _initLocation() async {
    ...
    _location?.getLocation().then((location) {
      if (location.latitude != null && location.longitude != null) {
        setState(() {
          _currentLocation = location;
          _lastRecordedPosition = LatLng(location.latitude!, location.longitude!);
        });
    ...
    // Start recording timer for every 1 minute
    _startRecordingTimer();

    // Listen for location updates to update UI only
    _location?.onLocationChanged.listen((newLocation) {
      if (newLocation.latitude != null && newLocation.longitude != null) {
        setState(() {
          _currentLocation = newLocation;
        ...
        // Move camera to current position (optional)
        moveToPosition(currentLatLng);
      }
    });
  }
  ...
  void _startRecordingTimer() {
    ...
    // Create a new timer that fires every 60 seconds (1 minute)
    _recordingTimer = Timer.periodic(const Duration(seconds: SAVE_INTERVAL_SECONDS), (timer) {
    ...
    debugPrint('Recording started - saving location every $SAVE_INTERVAL_SECONDS seconds');
  }
  ...
  Future<void> _recordCurrentLocation() async {
    if (_currentLocation == null || 
      ...
      debugPrint('No location available to record');
      return;
    }
    ...
    // This prevents saving if the user hasn't moved
    if (_lastRecordedPosition != null) {
      ...
      if (distance < 10) {
        debugPrint('Location changed only ${distance.toStringAsFixed(1)} meters, skipping save');
        return;
      }
    }

    await _saveLocationToDatabase(lat, lng);
    _lastRecordedPosition = LatLng(lat, lng);
    _lastSaveTime = DateTime.now();
  }
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          ...
          IconButton(
            icon: const Icon(Icons.list),
            onPressed: () => _getAllLocations(context),
            tooltip: 'View All Locations',
          ),
          IconButton(
            icon: const Icon(Icons.refresh),
                  ...
                  content: Column(
                    ...
                    children: [
                      ...
                      if (_locationPoints.isNotEmpty) ...[
                        ...
                        Text('End: ${_formatTimestamp(_locationRecords.last['timestamp'] as String)}'),
                        const SizedBox(height: 8),
                        Text('Recording: ${_isRecording ? "Active" : "Paused"}'),
                        Text('Interval: $SAVE_INTERVAL_SECONDS seconds'),
                        if (_lastSaveTime != null)
                          Text('Last Save: ${_formatTimestamp(_lastSaveTime!.toIso8601String())}'),
                      ],
                    ],
                  ),
                  actions: [
                ...
                children: [
                  ...
                  Row(
                    children: [
                      ...
                      Text(
                        _isRecording ? 'Recording every $SAVE_INTERVAL_SECONDSs' : 'Paused',
